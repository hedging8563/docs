...
Many media endpoints are asynchronous. A create request starts work and returns a public AI Sonar task identity; your application polls until that task reaches a terminal status. Build customer workflows only around the documented task ID and status fields.
...
| Final media URL missing | Task is not completed, or the job completed without usable output | Keep polling until terminal, then handle missing output as failed |
