...
```json
{
  "error": {
    ...
    "hint": "Insufficient balance: need ~$0.3500 for claude-sonnet-4-6, but balance is $0.1200. Try a cheaper model or contact support to add balance."
  }
}
```
...
Do not infer native protocol availability from a model or provider name, or from Chat response headers. Before choosing a native endpoint, read `GET /v1/models/{model}` and use only a format in `aisonar.accepted_request_formats`. Native requests use only the advertised protocol; temporary service availability is still decided at request time.
...
| `cache_pricing` | object or `null` | Model-specific prompt cache prices. |
