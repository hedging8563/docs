...
Structured outputs and tool calls are route-specific request formats. AI Sonar forwards several public API formats, so the safest production pattern is to keep each provider-native tool shape on the route that owns it and validate model output in your own application.
...
The stable shared validation for Chat Completions accepts `text` and `json_object`. `json_schema`, `strict`, and provider-specific schema enforcement may exist for some native formats or compatibility paths, but they are not a blanket promise across every AI Sonar route and model. Verify them against the selected model before relying on them.
