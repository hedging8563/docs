...
`GET /v1/management/api-keys/{keyId}/usage` can filter by `scene`, `accessChannel`, `model`, `modelVendor`, `startDate`, and `endDate`. Use these records instead of scraping dashboard pages or relying on provider task IDs.
...
Do not include API keys, management tokens, private media, full prompts, provider URLs, channel IDs, or private diagnostic identifiers unless AI Sonar support explicitly asks for a redacted sample.
