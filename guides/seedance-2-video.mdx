...
2. Send the user to the returned `h5_link`. The `session_id` and H5 link expire after 120 seconds. The `verification_token` is returned only by the AI Sonar callback redirect.
3. After the H5 flow completes, AI Sonar redirects to your `callback_url` with `session_id`, `verification_token`, `verification_status`, and `result_code`.
...
Optional: use the [test console](https://console.aisonar.dev/seedance-assets) to verify your request and callback flow, inspect material groups, and review verification history. Your production integration should call the APIs directly.
...
For `aigc_avatar`, `group_id` is optional; AI Sonar uses or creates the organization default virtual-avatar group. For `liveness_face`, `group_id` is required and must be the group returned by [Bind Real-Person Verification Result](/api-reference/video/bind-real-person-verification-result).
...
The `asset_url` field describes the upload source or temporary inspection information. Use the returned AI Sonar material asset `id` for generation; do not treat `asset_url` as a long-term material address.
