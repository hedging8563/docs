...
**Gemini Files and Cache:** `/upload/v1beta/files`, `/v1beta/files`, `/v1beta/files:register`, and `/v1beta/cachedContents` are available on the native Gemini route. Files use Gemini File API-compatible service channels; explicit cache resources can also route through Vertex AI channels. Resources created through AI Sonar are bound to the same service channel/key for later `generateContent` calls.
...
If AI Sonar cannot route a request with native tools to a native-capable provider path, it returns an explicit unsupported-field error instead of dropping the tool or pretending it is a Chat Completions function. User-defined function tools remain the portable path.
