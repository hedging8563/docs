...
Use [Create Material Asset Group](/api-reference/video/create-material-asset-group) to create an `aigc_avatar` group. For REST asset creation, `group_id` is optional; AI Sonar uses or creates the organization's default virtual-avatar group when it is omitted.
...
Deleting a group also deletes the AI Sonar materials it contains and cannot be undone.
...
3. Store the returned `Result.GroupId`. AI Sonar creates and binds the `LivenessFace` group during this step.
...
The source URL is only used for import. AI Sonar returns its own asset ID and keeps the asset in your organization's material library until you delete the asset or its group.
...
curl https://api.aisonar.dev/v1/videos/assets/asset-20260720123457-def45?project_name=default \
  -H "Authorization: Bearer $API_KEY"
```
...
```bash
curl 'https://api.aisonar.dev/?Action=CreateVisualValidateSession&Version=2024-01-01' \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"CallbackURL":"https://yourapp.example.com/seedance/callback","ProjectName":"default"}'

curl 'https://api.aisonar.dev/?Action=GetVisualValidateResult&Version=2024-01-01' \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl 'https://api.aisonar.dev/?Action=CreateAsset&Version=2024-01-01' \
  -H "Authorization: Bearer $API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
