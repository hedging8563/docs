...
Start with model discovery, then inspect the selected model's AI Sonar model details:
...
- `gpt-image-2` style requests use OpenAI-like `size`, `quality`, and edit fields. For generation and edits, `background` accepts `auto` or `opaque`; `transparent` is not supported. Leave optional fields out when you want the model or AI Sonar to use automatic defaults.
