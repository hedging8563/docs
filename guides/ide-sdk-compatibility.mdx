...
| **Gemini CLI** | Best-effort / experimental | Gemini | Custom AI Sonar base URL flow is not an official Gemini CLI integration |
...
| **OpenHands** | Supported | OpenAI-compatible | Use the LiteLLM/OpenAI-compatible path with `openai/<model>` and AI Sonar's `/v1` base URL |
...
| **OpenAI Agents SDK** | Python | Supported chat-completions path | Use `AsyncOpenAI(base_url="https://api.aisonar.dev/v1")` with `OpenAIChatCompletionsModel`; native Responses behavior depends on the selected model and route |
| **AutoGen** | Python | Supported custom endpoint path | Use `OpenAIChatCompletionClient` with `base_url: https://api.aisonar.dev/v1` |
| **Semantic Kernel** | .NET | Supported custom endpoint path | Use the OpenAI chat completion connector custom endpoint and point it at `https://api.aisonar.dev/v1` |
| **CrewAI** | Python | Supported custom endpoint path | Set `OPENAI_API_BASE` or `LLM(base_url=...)` to AI Sonar's `/v1` endpoint |
| **Pydantic AI** | Python | Supported OpenAI-compatible provider path | Use `OpenAIChatModel` with `OpenAIProvider(base_url="https://api.aisonar.dev/v1")` |
| **DSPy** | Python | Supported custom endpoint path | Use `dspy.LM("openai/<model>", api_base="https://api.aisonar.dev/v1")` |
| **Langflow** | Python / Web | Supported with scope limits | Use the OpenAI component's **OpenAI API Base** field with AI Sonar's `/v1` endpoint |
| **Haystack** | Python | Supported custom endpoint path | Use `OpenAIChatGenerator(api_base_url="https://api.aisonar.dev/v1")` |
| **Graphiti** | Python | Supported custom endpoint path | Use `OpenAIGenericClient` with AI Sonar's `/v1` endpoint |
...
| **Flowise** | Web / Node | Supported with scope limits | Use a ChatOpenAI-compatible node pointed at `https://api.aisonar.dev/v1` |
| **Mem0** | Python | Supported custom endpoint path | Keep the OpenAI provider and set `openai_base_url` to AI Sonar's `/v1` endpoint |
| **Agno** | Python | Supported path | Use Agno's OpenAI-compatible model path with AI Sonar's base URL |
| **Browser Use** | Python | Supported custom endpoint path | Use `ChatOpenAILike` with AI Sonar's `/v1` endpoint |
| **VoltAgent** | TypeScript | Supported custom endpoint path | Pass an AI SDK OpenAI provider configured with `baseURL: "https://api.aisonar.dev/v1"` |
| **Ragas** | Python | Supported custom endpoint path | Pass `AsyncOpenAI(base_url="https://api.aisonar.dev/v1")` into `llm_factory` |
| **Guardrails** | Python | Supported validation path | Pass an OpenAI SDK chat completions callable configured with AI Sonar's `/v1` endpoint |
| **Prompt flow** | Python / CLI | Supported custom endpoint path | Create an OpenAI connection with `base_url=https://api.aisonar.dev/v1` |
| **Promptfoo** | CLI / Node | Supported path | Use OpenAI chat or Responses providers with `apiBaseUrl: https://api.aisonar.dev/v1` |
| **Portkey Gateway** | Gateway | Supported path | Use Portkey custom-host routing with provider `openai` for OpenAI-compatible chat traffic |
| **Helicone** | Gateway / Observability | Supported observability path | Route OpenAI-compatible requests through Helicone Gateway with `Helicone-Target-Url: https://api.aisonar.dev` |
| **Langfuse** | Observability | Supported tracing path | Configure the Langfuse OpenAI integration with AI Sonar's `/v1` base URL |
| **OpenLIT** | Observability | Supported tracing path | Instrument an OpenAI SDK client configured with AI Sonar's `/v1` base URL |
| **OpenLLMetry** | Observability | Supported tracing path | Use OpenAI SDK instrumentation with AI Sonar's `/v1` base URL |
| **Phoenix** | Observability | Supported tracing path | Auto-instrument the OpenAI SDK client configured with AI Sonar's `/v1` base URL |
| **Opik** | Observability | Supported tracing path | Wrap an OpenAI SDK client configured with AI Sonar's `/v1` base URL |
| **LangBot** | Bot Platform | Supported chat path | Configure an OpenAI-compatible requester with AI Sonar's `/v1` endpoint |
| **Open WebUI** | Web Chat | Supported chat-app path | Add AI Sonar as an OpenAI-compatible API connection and refresh the model list |
