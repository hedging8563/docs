...
良好的 AI Sonar 可观察性始于公共标识符。您的日志应帮助您回答“用户请求了什么，AI Sonar 创建了什么公共任务，以及如何计费？”而不暴露敏感供应商细节或敏感用户数据。
...
`GET /v1/management/api-keys/{keyId}/usage` 可以按 `scene`、`accessChannel`、`model`、`modelVendor`、`startDate` 和 `endDate` 进行过滤。使用这些记录，而不是抓取仪表板页面或依赖供应商任务 ID。
...
除非 AI Sonar 支持明确要求提供已编辑的示例，否则请勿包含 API 密钥、管理令牌、私有媒体、完整提示、供应商 URL、通道 ID 或私有调试标识符。
