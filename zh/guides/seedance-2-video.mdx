...
对于 `aigc_avatar`，`group_id` 是可选的；AI Sonar 将使用或创建组织默认的虚拟人像组。对于 `liveness_face`，`group_id` 是必需的，且必须是 [绑定真人验证结果](/zh/api-reference/video/bind-real-person-verification-result) 返回的组 ID。
