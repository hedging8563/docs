...
当所选 Seedance 模型可使用 AI Sonar 素材库时，图片 URL 和受支持的内联 data URL 会在生成前自动准备为可复用的 AI Sonar 素材。如果准备超过 60 秒，请在返回的 `auto_material_asset_ids` 变为 `ACTIVE` 后重试。如果所选模型暂不可使用素材库，普通图片输入仍走常规图片路径。
...
`model` 应使用 AI Sonar 展示的模型 ID，再用 `operation` 和对应媒体输入选择操作能力。示例包括 `wan-2.7`、`happyhorse-1.0`、`viduq3`、`viduq3-mix`、`pixverse-v6`、`kling-3.0-video`、`veo3.1`、`seedance-2.0`；不要把提供商的操作名称当成 AI Sonar 模型名。
...
对于生产媒体输入，优先使用公共 `https` URL 而不是内联 `data:` URL。如果使用临时访问 URL，请确保它在 AI Sonar 完成任务创建前保持有效。
...
跨模型视频生成建议使用 `/v1/videos/generations`。如果你正在迁移已有的 Seedance 2.0 集成，并且请求已经是火山风格的 `content[]` 或 Action 形式，可以使用 `/api/v3` 下的 Seedance 兼容入口。两种入口都使用 AI Sonar Bearer API Key 和异步轮询，但请求与响应结构不同。

## OpenAI 风格和火山兼容视频 API

跨模型视频生成请使用 AI Sonar 统一的 `/v1/videos/generations`。如果你正在迁移已经使用火山风格 `content[]` 或 Action 请求的 Seedance 2.0 集成，可以使用 `/api/v3` 下的 Seedance 兼容入口。两种入口都使用 AI Sonar Bearer API Key 和异步轮询，但请求与响应结构不同。
