...
| **Gemini CLI** | 尽力支持 / 实验性 | Gemini | 自定义 AI Sonar 基础 URL 流不是官方 Gemini CLI 集成 |
...
| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| `stream_options` | object | 仅 Chat Completions：使用 `{ include_usage: true }` 可获取 token 计数 |
...
支持 `stream: true` 的生成端点（包括 Chat Completions 和 Responses）使用 Server-Sent Events (SSE)：
...
<AccordionGroup>
  ...
  <Accordion title="仅在 Chat Completions 中使用 stream_options.include_usage">
    Chat Completions 流式请求可启用 `stream_options.include_usage` 以获得准确的 token 计数。Responses 有自己的流式契约，不公开这个 Chat 专用选项。
  </Accordion>
