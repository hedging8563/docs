...
AI Sonar 提供了多种 API 格式，以便常见的编码工具、SDK 和框架能够以最少的胶水代码进行集成。

本页面特意比营销矩阵更为精简：

- **支持 (Supported)**：意味着我们记录了具体的设置路径，且 AI Sonar 提供了该路径所期望的协议形态。
- **强原生路径 (Strong native path)**：意味着代码库中还包含针对该协议族的直接适配器或请求格式证据。
- **尽力而为 (Best-effort)**：意味着集成可以工作，但客户端并未将此自定义网关工作流视为稳定契约。
...
| **Gemini CLI** | 尽力而为 / 实验性 | Gemini | 自定义 AI Sonar 基础 URL 流程并非官方 Gemini CLI 集成 |
| **OpenCode** | 支持 | OpenAI 兼容 | 默认使用 OpenAI 兼容提供商；仅在明确需要时切换到基于 Responses 的提供商 |
| **OpenHands** | 支持 | OpenAI 兼容 | 使用 LiteLLM/OpenAI 兼容路径，配置 `openai/<model>` 和 AI Sonar 的 `/v1` 基础 URL |

<Note>
  其他 OpenAI 兼容的编辑器和转发工具通常也适用于相同的基本 URL 模式；在生产环境使用前，请检查工具自身的自定义提供商支持情况。
</Note>
...
<Tabs>
  <Tab title="Cursor">
    ...
    ```
    Cursor 内部使用 Anthropic 风格的工具格式。AI Sonar 同时支持：
...
| **LlamaIndex** | Python | 通过 `OpenAILike` 支持 | 对于 AI Sonar 等第三方网关，请使用 `OpenAILike`，而非内置的 OpenAI 类 |
| **OpenAI Agents SDK** | Python | 支持 chat-completions 路径 | 使用 `AsyncOpenAI(base_url="https://api.aisonar.dev/v1")` 配合 `OpenAIChatCompletionsModel`；原生 Responses 行为取决于所选模型和路由 |
| **AutoGen** | Python | 支持自定义端点路径 | 使用 `OpenAIChatCompletionClient` 并设置 `base_url: https://api.aisonar.dev/v1` |
| **Semantic Kernel** | .NET | 支持自定义端点路径 | 使用 OpenAI 聊天完成连接器的自定义端点，并指向 `https://api.aisonar.dev/v1` |
| **CrewAI** | Python | 支持自定义端点路径 | 将 `OPENAI_API_BASE` 或 `LLM(base_url=...)` 设置为 AI Sonar 的 `/v1` 端点 |
| **Pydantic AI** | Python | 支持 OpenAI 兼容提供商路径 | 使用 `OpenAIChatModel` 配合 `OpenAIProvider(base_url="https://api.aisonar.dev/v1")` |
| **DSPy** | Python | 支持自定义端点路径 | 使用 `dspy.LM("openai/<model>", api_base="https://api.aisonar.dev/v1")` |
| **Langflow** | Python / Web | 支持（有限制） | 使用 OpenAI 组件的 **OpenAI API Base** 字段，填入 AI Sonar 的 `/v1` 端点 |
| **Haystack** | Python | 支持自定义端点路径 | 使用 `OpenAIChatGenerator(api_base_url="https://api.aisonar.dev/v1")` |
| **Graphiti** | Python | 支持自定义端点路径 | 使用 `OpenAIGenericClient` 配合 AI Sonar 的 `/v1` 端点 |
| **Dify** | - | 支持（有限制） | OpenAI 提供商和面向 chat-completions 的流程是预期路径；不适用于 Codex 特定的 Responses 或 WebSocket 行为 |
| **Flowise** | Web / Node | 支持（有限制） | 使用指向 `https://api.aisonar.dev/v1` 的 ChatOpenAI 兼容节点 |
| **Mem0** | Python | 支持自定义端点路径 | 保留 OpenAI 提供商并将 `openai_base_url` 设置为 AI Sonar 的 `/v1` 端点 |
| **Agno** | Python | 支持路径 | 使用 Agno 的 OpenAI 兼容模型路径，并将基础 URL 指向 AI Sonar |
| **Browser Use** | Python | 支持自定义端点路径 | 使用 `ChatOpenAILike` 配合 AI Sonar 的 `/v1` 端点 |
| **VoltAgent** | TypeScript | 支持自定义端点路径 | 传入配置了 `baseURL: "https://api.aisonar.dev/v1"` 的 AI SDK OpenAI 提供商 |
| **Ragas** | Python | 支持自定义端点路径 | 将 `AsyncOpenAI(base_url="https://api.aisonar.dev/v1")` 传入 `llm_factory` |
| **Guardrails** | Python | 支持验证路径 | 传入配置了 AI Sonar `/v1` 端点的 OpenAI SDK 聊天完成可调用对象 |
| **Prompt flow** | Python / CLI | 支持自定义端点路径 | 创建一个 `base_url=https://api.aisonar.dev/v1` 的 OpenAI 连接 |
| **Promptfoo** | CLI / Node | 支持路径 | 使用 `apiBaseUrl: https://api.aisonar.dev/v1` 的 OpenAI 聊天或 Responses 提供商 |
| **Portkey Gateway** | Gateway | 支持路径 | 使用 Portkey 自定义主机和 `openai` provider 转发 OpenAI 兼容聊天流量 |
| **Helicone** | Gateway / Observability | 支持可观测性路径 | 通过 Helicone Gateway 路由 OpenAI 兼容请求，并设置 `Helicone-Target-Url: https://api.aisonar.dev` |
| **Langfuse** | Observability | 支持追踪路径 | 使用 AI Sonar 的 `/v1` 基础 URL 配置 Langfuse OpenAI 集成 |
| **OpenLIT** | Observability | 支持追踪路径 | 对配置了 AI Sonar `/v1` 基础 URL 的 OpenAI SDK 客户端进行插桩 |
| **OpenLLMetry** | Observability | 支持追踪路径 | 使用配置了 AI Sonar `/v1` 基础 URL 的 OpenAI SDK 插桩 |
| **Phoenix** | Observability | 支持追踪路径 | 自动插桩配置了 AI Sonar `/v1` 基础 URL 的 OpenAI SDK 客户端 |
| **Opik** | Observability | 支持追踪路径 | 包装配置了 AI Sonar `/v1` 基础 URL 的 OpenAI SDK 客户端 |
| **LangBot** | Bot Platform | 支持聊天路径 | 使用 AI Sonar 的 `/v1` 端点配置 OpenAI 兼容请求器 |
| **Open WebUI** | Web Chat | 支持聊天应用路径 | 添加 AI Sonar 作为 OpenAI 兼容的 API 连接并刷新模型列表 |
...
<AccordionGroup>
  ...
  <Accordion title="使用适当的 tool_choice 格式">
    匹配 SDK 预期的格式。AI Sonar 同时接受 OpenAI 和 Anthropic 格式。
  </Accordion>
</AccordionGroup>
