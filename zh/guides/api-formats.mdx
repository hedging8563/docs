...
**Gemini Files 和 Cache：** 原生 Gemini 路径支持 `/upload/v1beta/files`、`/v1beta/files`、`/v1beta/files:register` 和 `/v1beta/cachedContents`。Files 使用兼容 Gemini File API 的服务端渠道；显式 Cache 资源也可以走 Vertex AI 渠道。通过 AI Sonar 创建的资源会绑定到同一个服务端渠道/key，后续 `generateContent` 会沿用该绑定。
...
如果 AI Sonar 无法把带原生工具的请求路由到支持原生格式的提供商路径，会返回明确的 unsupported-field 错误，而不是静默丢弃工具或伪装成 Chat Completions 函数。用户自定义函数工具仍然是最可移植的工具路径。
