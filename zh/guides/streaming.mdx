...
流式会逐步交付输出。只有模型公开 Responses 原生契约且存在同协议 service 路径时，才使用 Responses streaming；否则请使用 Chat Completions 或模型公开的原生协议。
## 推荐：Responses 流式传输
...
Responses SSE 会保留 service 事件名称、顺序和字段，只在旁路读取 usage 和终态，不改写 wire。第一个事件交付后，绝不重试该请求。
