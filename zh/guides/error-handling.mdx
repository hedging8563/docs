...
所有错误返回一致的 JSON 格式，并可选包含 [Agent-First hints](/guides/agent-first-api)：
...
必需的基础字段（`message`, `type`）始终存在；`code` 和 `param` 是可选字段，仅在适用时返回。提示字段（`did_you_mean`, `suggestions`, `hint`, `retryable`, `retry_after`, `balance_usd`, `estimated_cost_usd`）是用于 AI agent 自我纠正的可选扩展。详情见 [Agent-First API guide](/guides/agent-first-api)。

兼容 OpenAI 的端点使用 AI Sonar 的稳定网关错误类型。兼容 Anthropic 和 Gemini 的端点使用它们各自原生的错误类别和响应格式。
...
| 类型 | 说明 |
|------|------|
| `server_error` | 服务暂时无法完成请求 |
| `timeout_error` | 请求超时 |

仅当 `retryable` 为 `true` 时重试；如果响应包含 `retry_after`，请等待指定秒数。可根据 `alternatives` 改用其他可用模型。
