...
<Info>
对于 Seedance 2.0 模型，AI Sonar 会在生成前自动将图片字段（`image`、`image_url`、`image_urls`、`reference_images`、`start_image`、`end_image`）准备为可复用的虚拟数字人素材。如果 60 秒内仍未准备完成，API 会返回 `409 seedance_material_preparing` 和 `auto_material_asset_ids`；请在这些素材变为 `ACTIVE` 后重试。
</Info>
