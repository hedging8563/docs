...
音频输出是否开启取决于具体模型。在 AI Sonar 中，Veo 3 和 Seedance 请求在省略 `output_audio` 时默认按开启音频处理。模型支持音频控制时，可用 `output_audio` 显式切换；兼容别名 `outputAudio` 和 `generate_audio` 也可以使用，但多个字段同时出现时值必须一致。
...
<ParamField body="image_url" type="string">
  用于图生视频的起始图片 URL。对于 Seedance 素材，也可以传入 `asset://asset-YYYYMMDDHHMMSS-xxxxx`，把一个处于 `ACTIVE` 状态的 AI Sonar 素材用作首帧。为了获得最广泛的跨模型兼容性，普通图片建议使用 `https` URL。
</ParamField>
...
用于参考图生视频的参考图输入。可传数量取决于模型。对于 `seedance-2.0` 和 `seedance-2.0-fast`，AI Sonar 当前支持最多 9 张参考图，外加最多 3 段参考视频和 3 段参考音频。每张 Seedance 参考图既可以是 `https` URL，也可以是指向 `ACTIVE` AI Sonar 素材的 `asset://asset-YYYYMMDDHHMMSS-xxxxx`。模型选型、4K 边界和 Mini 说明请参考 [Seedance 2.0 视频模型指南](/zh/guides/seedance-2-video)。普通图片建议优先使用公网 `https` URL；兼容模型也支持内联 `data:` URL。 对于 `grok-imagine-video`，reference-to-video 最多接受 7 个图片参考，且 `duration` 最高为 10 秒。`grok-imagine-video-1.5-preview` 仅支持图生视频，不接受参考图片。
</ParamField>

<ParamField body="material_asset_id" type="string">
  [创建素材](/zh/api-reference/video/create-material-asset)或自动图片准备流程返回的 AI Sonar Seedance 素材 ID。素材变为 `ACTIVE` 后，可在能够使用 AI Sonar 素材库的 Seedance 模型中使用。该字段只表示 `reference-to-video` 的通用参考素材；省略 `operation` 时，AI Sonar 会据此自动推断为 `reference-to-video`。若要把素材作为 `image-to-video` 首帧，请改用 `image_url: "asset://<material_asset_id>"`；首尾帧流程使用 `start_image` 和 `end_image`，明确的参考图角色使用 `reference_images`。
</ParamField>

<ParamField body="material_asset_ids" type="array">
  多个 AI Sonar Seedance 素材 ID，仅用于 `reference-to-video` 的通用参考素材。它们与 `reference_images` 共用 Seedance 图片参考数量限制；所选模型必须能够使用 AI Sonar 素材库。若要把其中一个素材作为 `image-to-video` 首帧，请选取该素材并改用 `image_url: "asset://<material_asset_id>"`。
</ParamField>

<Info>
当所选 Seedance 模型可使用 AI Sonar 素材库时，AI Sonar 会在生成前将图片字段（`image`、`image_url`、`image_urls`、`reference_images`、`start_image`、`end_image`）准备为可复用素材。这些字段也接受 `asset://asset-YYYYMMDDHHMMSS-xxxxx` 形式的 AI Sonar 素材 URI；AI Sonar 会校验素材是否属于当前组织，并保留字段所表达的用途。原始服务端 `asset://` ID 会被拒绝。如果 60 秒内仍未准备完成，API 会返回 `409 seedance_material_preparing` 和 `auto_material_asset_ids`；请在这些素材变为 `ACTIVE` 后重试。如果所选模型暂不可使用素材库，普通图片输入仍走常规图片路径；显式素材 ID 会返回素材可用性错误。
</Info>
...
<ParamField body="safety_identifier" type="string">
  兼容视频模型的终端用户安全标识。Seedance 未传该字段时，AI Sonar 会使用 `user` 的值。
</ParamField>
...
<ParamField body="user" type="string">
  终端用户的唯一标识符。对 Seedance，如果未传 `safety_identifier`，AI Sonar 会使用该值。
</ParamField>

## 兼容说明

- 规范公开字段继续使用 snake_case：`aspect_ratio`、`output_audio`、`reference_images` 和 `reference_image_type`。
- 为了兼容已有调用，AI Sonar 也接受 `ratio`、`generate_audio`、`outputAudio`、`seconds`、`referenceImages` 和 `referenceImageType`。
- 如果规范字段和别名字段同时出现，值必须一致；冲突会在创建任务前被拒绝。
- 如果省略 `operation`，AI Sonar 会根据输入自动推断操作；生产环境仍建议显式传入。
...
对于 Seedance 1.5/2.0 模型，统一接口以 AI Sonar 字段为主，同时接受兼容别名 `seconds`、`ratio` 和 `generate_audio`。省略 Seedance 参数时会使用这些默认值：`duration=5`、`resolution=720p`、`aspect_ratio=adaptive`、`output_audio=true`、`watermark=false`、`return_last_frame=false`、`execution_expires_after=172800`、`priority=0`、`seed=-1`。

`duration=-1` 或 `seconds=-1` 表示让 Seedance 在模型支持范围内自动选择输出时长。AI Sonar 会在任务完成前按保守方式预估费用，并在可获得完成任务 usage 时按实际结果结算。`service_tier=default` 对 Seedance 2.0 作为兼容 no-op 接受；`service_tier=flex`、`frames`、`camera_fixed` 会在所选模型不支持时被拒绝。
