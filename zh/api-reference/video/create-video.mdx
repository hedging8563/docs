...
<ParamField body="material_asset_id" type="string">
  [创建素材](/zh/api-reference/video/create-material-asset)或自动图片准备流程返回的 AI Sonar Seedance 素材 ID。素材变为 `ACTIVE` 后，可在能够使用 AI Sonar 素材库的 Seedance 模型中使用。该字段表示通用参考素材；如果要明确指定首帧、尾帧或参考图，请把 `asset://<material_asset_id>` 分别放入 `start_image`、`end_image` 或 `reference_images`。
</ParamField>

<ParamField body="material_asset_ids" type="array">
  多个 AI Sonar Seedance 素材 ID。它们与 `reference_images` 共用 Seedance 图片参考数量限制；所选模型必须能够使用 AI Sonar 素材库。
</ParamField>
