...
H5 视觉验证流程结束后轮询此 Action。成功结果会返回已验证人员对应的 AI Sonar 素材组 ID。
...
错误使用 `ResponseMetadata.Error.Code` 和 `ResponseMetadata.Error.Message`。成功响应包含 `ResponseMetadata` 和 `Result`，不使用 AI Sonar 其他 `/v1` API 的响应结构。
