...
在上传可复用的 `liveness_face` 素材前，先创建真人视觉验证会话。AI Sonar 使用火山引擎 Action API 的请求格式，并使用 AI Sonar Bearer 认证。
...
请将 `Result.H5Link` 视为不透明的浏览器 URL：以顶层页面打开，不要解析、解码、改写或嵌入。浏览器交接和回调桥接由 AI Sonar 负责。
...
错误也使用相同的顶层结构。请读取 `ResponseMetadata.Error.Code` 和 `ResponseMetadata.Error.Message`，不要按 AI Sonar 其他 `/v1` API 的错误字段处理。
