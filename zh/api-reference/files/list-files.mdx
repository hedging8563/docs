...
此端点主要服务于 AI Sonar 的 OpenAI 兼容 Batch API，包括上传的批量输入文件以及生成的输出或错误文件。

当请求选择 Anthropic Files 兼容模式时，同一路由也可以列出绑定工作区中可用的 Anthropic 风格文件。

不带 Anthropic 请求头时，列表可能同时包含批量文件和 AI Sonar 图像/参考文件。可用 `purpose=batch`、`purpose=batch_output`、`purpose=user_data` 或 `purpose=vision` 缩小范围。
## 注意事项
