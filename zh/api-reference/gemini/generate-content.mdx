...
AI Sonar 只对公开 Gemini 原生契约且具备同协议 service 路径的模型提供该入口，绝不回落 Chat Completions。ProtoJSON lowerCamelCase 和原始 proto snake_case 名称都是官方拼写，混合请求也会原样保留。未知字段会 best-effort 透传，由 service 决定是否支持。
## 路径参数
...
<ParamField body="contents" type="array" required>
  对话内容。`role` 值和所有 `parts` 都会保持客户端原样；AI Sonar 不做 trim/lowercase、Base64 改写或 MIME 推断。`inlineData` / `inline_data` 和 `fileData` / `file_data` 都是官方拼写，AI Sonar 不会在本地强制要求 `fileData.mimeType`。引用 AI Sonar 创建的 Files/Cache ID 时会检查所有权并保留原 service 绑定；其他媒体和工具规则交给厂商判断。
</ParamField>
...
<ParamField body="generationConfig" type="object">
  原生生成配置。`maxOutputTokens`、`candidateCount`、`thinkingConfig` 等 lowerCamelCase 和对应 snake_case 名称都会原样接受。AI Sonar 不会在失败后删除 `thinkingConfig`，也不会对 streaming 的 `candidateCount` 设置本地限制；范围和组合由 service 判断。
</ParamField>
