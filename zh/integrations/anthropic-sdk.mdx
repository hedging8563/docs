---
title: "Anthropic SDK"
description: "使用 AI Sonar 与 Anthropic SDK，以兼容 Claude 原生 Messages API"
---

## 概述

AI Sonar 支持原生 Anthropic **Messages API** 路径，因此你可以直接将官方 Anthropic SDK 用于 Claude 模型。

<Note>
对于 Anthropic SDK，请使用 `https://api.aisonar.dev` 作为 base URL，不要自行追加 `/v1`。
</Note>
...
<AccordionGroup>
  <Accordion title="错误的 Base URL">
    - 使用 `https://api.aisonar.dev`
    - 配置 Anthropic SDK 时，不要手动追加 `/v1`
  </Accordion>

  <Accordion title="身份验证失败">
    - 检查你的 AI Sonar API key 是否以 `sk-` 开头
