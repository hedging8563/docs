---
title: "LangChain"
description: "使用标准 OpenAI 兼容的 chat 和 embeddings 将 AI Sonar 集成到 LangChain 中"
---

## 概述

当你使用标准 OpenAI 兼容的 chat 和 embeddings 接口时，AI Sonar 可以很好地与 LangChain 的 `ChatOpenAI` 和 `OpenAIEmbeddings` 集成配合使用。
...
<AccordionGroup>
  <Accordion title="显式传递 base_url">
    最可靠的 AI Sonar 配置方式是直接将 `base_url="https://api.aisonar.dev/v1"` 传递给 `ChatOpenAI` 和 `OpenAIEmbeddings`，而不是依赖旧的环境变量别名。
  </Accordion>
