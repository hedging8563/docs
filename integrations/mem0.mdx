---
title: "Mem0"
description: "Use AI Sonar as an OpenAI-compatible LLM provider in Mem0 OSS"
---

## Overview

Mem0 OSS can use AI Sonar through its OpenAI-compatible LLM configuration by keeping `provider: "openai"` and setting `openai_base_url`.

<Note>
**Type**: Memory Framework

**Primary Path**: OpenAI-compatible LLM configuration

**Support Confidence**: Supported custom endpoint path
</Note>

## Environment

```bash
export API_KEY="sk-your-api-key"
```

## Python Configuration

```python
import os

from mem0 import Memory

config = {
    "llm": {
        "provider": "openai",
        "config": {
            "model": "claude-sonnet-5",
            "api_key": os.environ["API_KEY"],
            "openai_base_url": "https://api.aisonar.dev/v1",
            "temperature": 0.1,
        },
    },
    "vector_store": {
        "provider": "qdrant",
        "config": {"host": "localhost", "port": 6333},
    },
}

memory = Memory.from_config(config)
```

## Basic Usage

```python
memory.add("The user prefers concise engineering summaries.", user_id="demo")
results = memory.search("How should I answer?", user_id="demo")
print(results)
```

## Endpoint Notes

This configuration covers Mem0's OpenAI-compatible LLM path. Configure embedders separately according to the vector store and embedding model you use.
