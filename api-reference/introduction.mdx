---
title: 'API Reference'
description: 'Complete reference for the AI Sonar API'
---
...
Get your API key from the [Dashboard](https://console.aisonar.dev).
...
<Info>
Use `/v1/realtime` for WebSocket upgrade requests. A plain `GET /v1/realtime` returns endpoint metadata for clients that cannot inspect WebSocket routes directly.
</Info>
...
| `channel.channelType` | `PLATFORM` (AI Sonar) or `PRIVATE` (BYOK) |
