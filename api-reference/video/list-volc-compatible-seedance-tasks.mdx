...
- `filter.task_ids`: repeat the query parameter for multiple exact task IDs; do not comma-join them.
...
```bash
curl "https://api.aisonar.dev/api/v3/contents/generations/tasks?page_num=1&page_size=20&filter.status=succeeded" \
  -H "Authorization: Bearer $API_KEY"
```

```bash
curl "https://api.aisonar.dev/api/v3/contents/generations/tasks?filter.task_ids=cgt-0123456789abcdef0123456789abcdef" \
  -H "Authorization: Bearer $API_KEY"
```
