...
Poll this Action after the H5 visual validation flow. A successful result returns the AI Sonar material group ID for the verified person.

## Endpoint And Authentication

```text
POST https://api.aisonar.dev/api/v3?Action=GetVisualValidateResult&Version=2024-01-01
...
Errors use `ResponseMetadata.Error.Code` and `ResponseMetadata.Error.Message`. A success response contains `ResponseMetadata` and `Result`; it does not use the response envelope from AI Sonar's other `/v1` APIs.
