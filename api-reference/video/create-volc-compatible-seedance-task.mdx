...
- Existing AI Sonar Action aliases are retained only for backward compatibility. They are not part of the official wire-compatible task contract.
...
- The REST body accepts only official top-level fields. AI Sonar-specific fields such as top-level `material_asset_id`, `material_asset_ids`, and `priority` are rejected. Put an AI Sonar material URI inside the official `image_url.url` object instead.
...
- `callback_url` may point to a public HTTP(S) endpoint. AI Sonar validates and stores it without adding it to the task-generation request.

## Callback Delivery

When `callback_url` is present, AI Sonar sends an HTTP `POST` whenever task status changes. Callback statuses are `queued`, `running`, `succeeded`, `failed`, and `expired`. The JSON body is exactly the task object returned by the get-task endpoint.

A `2xx` response acknowledges delivery. For `succeeded` and `failed`, a delivery that does not succeed within five seconds is retried up to three times, matching the official behavior. The callback has only the standard JSON content header and no AI Sonar-specific delivery headers. Redirects are not followed, and private or reserved network targets are rejected.
...
For production creates, send a unique `Idempotency-Key` header and persist it with the exact JSON request body. If the connection closes before you receive the create response, retry the same REST request with the same AI Sonar API credential, key, and unchanged body:

- If the original task was created, AI Sonar returns the same `cgt-...` ID and adds `Idempotency-Replayed: true`.
- If the original request is still being registered, AI Sonar returns `409 IdempotencyRequestInProgress` with `Retry-After`; retry the same key and body after that delay.
