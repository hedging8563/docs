...
Create the real-person visual validation session before uploading reusable `liveness_face` materials. AI Sonar uses the Volc Action API shape and AI Sonar Bearer authentication.

## Endpoint And Authentication

```text
POST https://api.aisonar.dev/api/v3?Action=CreateVisualValidateSession&Version=2024-01-01
...
Treat `Result.H5Link` as an opaque browser URL: open it as a top-level page, and do not parse, decode, rewrite, or embed it. AI Sonar owns the browser handoff and callback bridge.
...
```json
{
  ...
  "Result": {
    ...
    "H5Link": "https://api.aisonar.dev/api/v3/visual-validation/sessions/svv_example/h5?Version=2024-01-01",
    "CallbackURL": "https://yourapp.example.com/seedance/callback"
  }
}
```

Errors use the same top-level envelope. Read `ResponseMetadata.Error.Code` and `ResponseMetadata.Error.Message` instead of expecting the fields from AI Sonar's other `/v1` APIs.
