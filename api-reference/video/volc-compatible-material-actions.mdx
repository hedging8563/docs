...
These Actions let an existing Volcengine material client keep its Action names, `Version=2024-01-01`, PascalCase JSON bodies, filters, sorting, pagination, and `ResponseMetadata + Result` envelope. AI Sonar replaces only the endpoint and authentication method.

## Endpoint And Authentication

```text
POST https://api.aisonar.dev/?Action=<ACTION>&Version=2024-01-01
Authorization: Bearer <API_KEY>
Content-Type: application/json
```

`POST /api/v3?Action=<ACTION>&Version=2024-01-01` is also supported. All material Actions are POST-only. Volcengine AK/SK signatures are not accepted; a request with no valid AI Sonar Bearer key returns `401 InvalidCredential`.
...
```bash
curl 'https://api.aisonar.dev/?Action=CreateAssetGroup&Version=2024-01-01' \
...
AI Sonar's `/v1/videos/assets*` REST endpoints remain available. New AI Sonar-native integrations may prefer their snake_case bodies; existing Volcengine clients can use the Actions on this page without reshaping requests. See [Seedance Materials and Real-Person Verification](/guides/seedance-materials) for the complete asset lifecycle.
