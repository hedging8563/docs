...
For `library_type: "aigc_avatar"`, `group_id` is optional; AI Sonar uses or creates the organization default virtual-avatar group. For `library_type: "liveness_face"`, `group_id` is required and must be the AI Sonar group returned by [Bind Real-Person Verification Result](/api-reference/video/bind-real-person-verification-result).
