...
<Info>
For Seedance 2.0 models, AI Sonar automatically prepares image fields (`image`, `image_url`, `image_urls`, `reference_images`, `start_image`, `end_image`) as reusable virtual-avatar materials before generation. If preparation is not complete within 60 seconds, the API returns `409 seedance_material_preparing` with `auto_material_asset_ids`; retry after those material assets become `ACTIVE`.
</Info>
