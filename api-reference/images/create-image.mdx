...
For `gpt-image-2` image generation, supported public parameters are `prompt`, `n`, `size`, `quality`, `response_format`, `async`, `background`, `output_format`, `output_compression` or `compression`, `moderation`, and `user`. Omit `size` or `quality` to let AI Sonar use `auto`; custom `size` values must use the flexible `WIDTHxHEIGHT` contract documented below.
