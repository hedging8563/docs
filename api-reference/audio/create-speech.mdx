...
<ParamField body="model" type="string" default="tts-1">
  TTS model. OpenAI-compatible examples include `tts-1` (standard) and `tts-1-hd` (higher quality); MiniMax high-definition speech is available as `speech-2.8-hd`. Query `GET /v1/models?recommended_for=tts` for the current shortlist.
</ParamField>
